...
# Apply ARIMA methods to forcast number of DDoS attacks in next hour
X = y1
size = int(len(X) * 0.80)
...
plt.title('Trend of DDoS flows with HWES')
plt.xlabel('Steps')
...
# apply ARIMA methods to forcast number of Normal flows in next hour
X = y2
size = int(len(X) * 0.80)
train, test = X[0:size], X[size:len(X)]
history = [x for x in train]
...
x2=np.arange(size,len(X))
plt.figure(figsize=(12,5))
plt.title('Trend of Normal flows with HWES')
plt.xlabel('Steps')
